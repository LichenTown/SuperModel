...
const LOGGED_MODEL_IDS = new Set<string>();

// By default, this generator should run last.
export const generatorName = "Item Model Generator";
...
export default async function generate(packPath: string, buildPath: string) {
    ...
    const modelsAddedByType: Record<string, { parent?: string; folder: string; id: string; definition?: Record<string, unknown>; source: "file" | "queue" }[]> = {};
    ...
    const totalGeneratedModels = Object.values(summaryByType)
        .reduce((sum, list) => sum + list.length, 0);

    // Print final log for item processing.
    logProcess("Item", "white", `Generated ${totalModels} thresholds using a total of ${totalGeneratedModels} item model definition(s).`, console.log);
...
<head>
    ...
    <meta content="Supermodel - Item Model Summary" property="og:title" />
    <meta content="Pack: ${getConfig().packName}" property="og:description" />
    <meta content="https://lichentown.github.io/SuperModel/" property="og:url" />
    <meta content="#ffd343" data-react-helmet="true" name="theme-color" />
...
/* Replace variables for custom item model definitions. */
...
async function updateItemDefinition(mcItemsBase: string, type: string, modelRefs: ModelRef[], vanillaAssetsBase: string): Promise<ModelWithThreshold[]> {
    ...
    for (const ref of modelRefs) {
        ...
        const modelId = `${ref.folder}/${ref.id}`;
        const logKey = `${ref.source}:${modelId}`;
        previousIndex = generateUniqueThreshold(modelId);
        if (takenThresholds.has(previousIndex)) {
            if (!LOGGED_MODEL_IDS.has(logKey)) {
                LOGGED_MODEL_IDS.add(logKey);
                logProcess("Item", "orange", `Model "\u001b[30m${ref.folder}/\u001b[36m${ref.id}\u001b[0m" already present at threshold \u001b[33m${previousIndex}\u001b[0m.`);
            }
            continue;
        }
        ...
        if (!LOGGED_MODEL_IDS.has(logKey)) {
            LOGGED_MODEL_IDS.add(logKey);
            logProcess("Item", "white", `Applied model "\u001b[30m${ref.folder}/\u001b[36m${ref.id}\u001b[0m" at threshold \u001b[35m${previousIndex}\u001b[0m.`);
        }
